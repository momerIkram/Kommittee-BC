...
def days_between_specific_dates(start_month_idx, start_day_of_month, end_month_idx, end_day_of_month, base_year=2024):
    if start_month_idx > end_month_idx or (start_month_idx == end_month_idx and start_day_of_month >= end_day_of_month):
        return 0 # Payout happens before or on collection day for this installment
    ...
    try:
        # Clamp days to valid range for month
        _, max_days_start_month = pd.Timestamp(f'{start_actual_year}-{start_actual_month}-01').days_in_month, pd.Timestamp(f'{start_actual_year}-{start_actual_month}-01').days_in_month
        clamped_start_day = min(start_day_of_month, max_days_start_month)

        _, max_days_end_month = pd.Timestamp(f'{end_actual_year}-{end_actual_month}-01').days_in_month, pd.Timestamp(f'{end_actual_year}-{end_actual_month}-01').days_in_month
        clamped_end_day = min(end_day_of_month, max_days_end_month)

        date_start = date(start_actual_year, start_actual_month, clamped_start_day)
        date_end = date(end_actual_year, end_actual_month, clamped_end_day)
        
        if date_end < date_start: return 0 # Should not happen if initial check passes but defensive
        return (date_end - date_start).days
    except ValueError: # Fallback for other unexpected date issues
        # Simplified fallback if datetime objects fail for any reason
        num_full_months = (end_actual_year - start_actual_year) * 12 + (end_actual_month - start_actual_month)
        if num_full_months < 0: return 0
        if num_full_months == 0:
            return max(0, end_day_of_month - start_day_of_month)
        
        # Approximate days: full months between + days in start/end month
        days = (num_full_months -1) * 30.4375 # Avg days in month
        # Days remaining in start month
        days += (pd.Timestamp(f'{start_actual_year}-{start_actual_month}-01').days_in_month - start_day_of_month + 1)
        # Days into end month
        days += end_day_of_month
...
# === FEE SUGGESTION CALCULATOR ===
with st.expander("💡 Fee Suggestion Calculator", expanded=False):
    st.markdown("This tool helps estimate a suitable fee percentage for a ROSCA product based on your global risk, NII, and target profit margin settings.")
    fee_calc_cols = st.columns(2)
    configured_durations_for_main_sim_key = "main_durations" 
    configured_durations_for_main_sim = [int(d) for d in st.session_state.get(configured_durations_for_main_sim_key, [3,6,10])] # Default if key not in session
    
    selected_duration_for_calc = fee_calc_cols[0].selectbox(
        "Select Duration (Months) for Fee Calc", 
        options=configured_durations_for_main_sim if configured_durations_for_main_sim else [3,4,5,6,8,10], # Fallback options
    ...
    selected_slab_for_calc = fee_calc_cols[1].selectbox(
        "Select Slab Installment Amount for Fee Calc", options=standard_slabs_for_calc, index=2, key="fee_calc_slab")

    if selected_duration_for_calc and selected_slab_for_calc: # Ensure selections are made
        calc_d = int(selected_duration_for_calc); calc_slab = int(selected_slab_for_calc)
        ...
        derived_default_post_pct_calc = 100 - default_pre_pct # Use derived post_pct
        calc_avg_loss_per_user = ( (default_rate / 100) * (default_pre_pct / 100) * calc_loss_pre ) + \
                                 ( (default_rate / 100) * (derived_default_post_pct_calc / 100) * calc_loss_post )
        calc_avg_payout_slot = math.ceil(calc_d / 2) 
        calc_avg_payout_due_month_idx = 0 + calc_avg_payout_slot - 1 
        calc_expected_nii_per_user = 0
        calc_daily_rate = (kibor/100 + spread/100) / 365
        for j_inst_calc in range(calc_d):
            collection_month_idx_inst_calc = 0 + j_inst_calc
            days_held_inst_calc = days_between_specific_dates(
                collection_month_idx_inst_calc, global_collection_day, calc_avg_payout_due_month_idx, global_payout_day)
        ...
        if calc_total_commitment > 0: calc_suggested_fee_pct = (calc_suggested_total_fee_amount / calc_total_commitment) * 100
        calc_suggested_fee_pct = max(0, calc_suggested_fee_pct)
        st.markdown("---"); st.markdown(f"**For a {calc_d}M duration, {calc_slab:,} PKR slab product:**")
        ...
        st.caption(f"Uses global inputs: {default_rate}% DR, {global_pre_payout_recovery_pct}% Pre-Recov, {kibor}% KIBOR, {spread}% Spread, {global_target_profit_margin_pct}% Target Profit.")

# === DURATION/SLAB/SLOT CONFIGURATION ===
validation_messages = [] 
durations_input_main = st.multiselect("Select Durations (months) for Simulation", [3, 4, 5, 6, 8, 10], default=[3, 4, 6], key=configured_durations_for_main_sim_key)
durations = [int(d) for d in durations_input_main] 
yearly_duration_share = {}; slab_map = {}; slot_fees = {}; slot_distribution = {}; first_year_defaults = {}

def get_simple_fee_suggestion_text(d_config_val, current_kibor, current_spread, current_default_rate, current_default_pre_pct_val, current_recovery_pct):
    example_slab_sugg = 1000; total_commit_sugg = d_config_val * example_slab_sugg
    ...
    derived_default_post_pct_val = 100 - current_default_pre_pct_val
    pre_def_loss_sugg = total_commit_sugg * (current_default_rate/100) * (current_default_pre_pct_val / 100) * (1 - current_recovery_pct / 100)
    post_def_loss_sugg = total_commit_sugg * (current_default_rate/100) * (derived_default_post_pct_val / 100)
    avg_loss_sugg = pre_def_loss_sugg + post_def_loss_sugg
    simple_target_profit = total_commit_sugg * (global_target_profit_margin_pct / 100) 
    ...
    return f"Simp. Sugg. (1k slab, {global_target_profit_margin_pct}% profit): ≥ {max(0,suggested_pct):.2f}%"
...
for y_config in range(1, 6):
    with st.expander(f"Year {y_config} Duration Share"):
        ...
        if not durations: st.caption("Please select durations for simulation above to configure year shares.")
        ...
        if total_dur_share_config > 100: validation_messages.append(f"⚠️ Year {y_config} duration share total is {total_dur_share_config}%. Must not exceed 100%.")

if not durations: st.warning("Please select at least one duration for the simulation to proceed with slab and slot configuration.")
for d_config in durations: 
    with st.expander(f"{d_config}M Slab Distribution"):
        ...
        if total_slab_pct_config > 100: validation_messages.append(f"⚠️ Slab distribution for {d_config}M totals {total_slab_pct_config}%. Must not exceed 100%.")
    ...
    with st.expander(f"{d_config}M Slot Fees & Blocking"):
        ...
        for s_config in range(1, d_config + 1):
            ...
            fee_input_val = st.number_input(f"Slot {s_config} Fee % (on total commitment)", 0.0, 100.0, 1.0, key=key_fee_config, help=simple_help_text)
            blocked_input_val = st.checkbox(f"Block Slot {s_config}", key=key_block_config)
            slot_pct_input_val = st.number_input(label=f"Slot {s_config} % of Users (Duration {d_config}M)", min_value=0, max_value=100, value=0, step=1, key=key_pct_config)
...
for d_config in durations: 
    if d_config in slot_distribution and slot_distribution[d_config]:
        total_slot_dist_pct = sum(slot_distribution[d_config].values())
        if total_slot_dist_pct > 100: validation_messages.append(f"⚠️ Slot distribution for {d_config}M totals {total_slot_dist_pct}%. Must not exceed 100%.")
if validation_messages:
    for msg_val in validation_messages: st.warning(msg_val)
    st.stop()
if not durations: 
    st.info("Select durations and configure shares to run the forecast.")
    st.stop()

# === FORECASTING LOGIC ===
def run_forecast(config_param_fc):
    months_fc = 60; initial_tam_fc = int(config_param_fc['total_market'] * config_param_fc['tam_pct'] / 100)
    new_users_monthly_fc = [int(initial_tam_fc * config_param_fc['start_pct'] / 100)]
    rejoin_tracker_fc = {}; forecast_data_fc, deposit_log_data_fc, default_log_data_fc, lifecycle_data_fc = [], [], [], []
    TAM_used_cumulative_fc = new_users_monthly_fc[0]; TAM_current_year_fc = initial_tam_fc
    enforce_cap_growth_fc = config_param_fc.get("cap_tam", False)
    current_kibor_rate_fc = config_param_fc['kibor'] / 100; current_spread_rate_fc = config_param_fc['spread'] / 100
    ...
    global_default_pre_frac_fc_from_global = default_pre_pct / 100 # Use global value
    ...
    for m_idx_fc in range(months_fc):
        ...
        for dur_val_fc, dur_share_pct_fc in durations_for_this_year_fc.items():
            if dur_val_fc not in slab_map or not slab_map[dur_val_fc]: continue
            ...
            for installment_val_fc, slab_share_pct_fc in slab_map[dur_val_fc].items():
                ...
                if dur_val_fc not in slot_fees or not slot_fees[dur_val_fc] or \
                   dur_val_fc not in slot_distribution or not slot_distribution[dur_val_fc]: continue
                for slot_num_fc, slot_config_meta_fc in slot_fees[dur_val_fc].items():
                    ...
                    num_pre_payout_defaulters_fc = int(num_defaulters_total_fc * global_default_pre_frac_fc_from_global)
...
with pd.ExcelWriter(output_excel_main, engine="xlsxwriter") as excel_writer_main:
    # Check if 'durations' is empty, if so, don't run scenarios
    if not durations:
        st.error("No durations selected for simulation. Please select at least one duration in the 'DURATION/SLAB/SLOT CONFIGURATION' section.")
    else:
        for scenario_idx_main, scenario_data_main in enumerate(scenarios):
            ...
            if not df_forecast_main.empty:
                ...
                if mask_main.any():
                    safe_denominator = df_yearly_summary_main.loc[mask_main, "Total Default Loss (Lifetime)"].replace(0, np.nan)
                    df_profit_share_main.loc[mask_main, "% Loss Covered by External Capital"] = \
                        (df_yearly_summary_main.loc[mask_main, "External Capital For Loss (Lifetime)"] / safe_denominator).fillna(0) * 100
