...
for i in range(scenario_count):
    with st.sidebar.expander(f"Scenario {i+1} Settings"):
        ...
        start_pct = st.number_input("Starting TAM % (Month 1 New Users)", min_value=0.01, max_value=100.0, value=10.0, step=0.01, key=f"start_pct_{i}", help="Initial new users as % of initial TAM for Month 1.") # Clarified label
        monthly_growth = st.number_input("Monthly Acquisition Rate (on Cum. Acquired Base) (%)", value=2.0, key=f"growth_{i}", help="New users next month = Cum. Acquired Base * Rate") #MODIFIED LABEL
        ...
        scenarios.append({
            ...
            "start_pct": start_pct, "monthly_growth": monthly_growth, # This is now the AcquisitionRate
...
for d_config in durations:
    ...
    with st.expander(f"{d_config}M Slot Fees & Blocking"):
        ...
        for s_config in range(1, d_config + 1):
            ...
            avg_nii_sugg = total_commit_sugg * ((kibor + spread) / 100 / 12) * sum(range(1, d_config + 1)) / d_config
...
def run_forecast(config_param_fc):
    ...
    # --- NEW: Acquisition logic setup ---
    acquisition_rate_fc = config_param_fc['monthly_growth'] / 100  # User's "AcquisitionRate"
    initial_new_users_m1_fc = int(initial_tam_fc * (config_param_fc['start_pct'] / 100))
    cumulative_acquired_base_fc = 0  # Tracks sum of *actual new users acquired* so far
    # --- END: Acquisition logic setup ---
    
    rejoin_tracker_fc = {}
    forecast_data_fc, deposit_log_data_fc, default_log_data_fc, lifecycle_data_fc = [], [], [], []
    
    TAM_used_cumulative_fc = 0 # Tracks total users acquired against TAM ceiling
    TAM_current_year_fc = initial_tam_fc # TAM for the current operating year
    ...
    for m_idx_fc in range(months_fc): # 0-indexed month of simulation
        ...
        # --- NEW: Monthly New User Acquisition Calculation ---
        if m_idx_fc > 0 and m_idx_fc % 12 == 0: # Start of a new year (Year 2 onwards)
            TAM_current_year_fc = int(TAM_current_year_fc * (1 + config_param_fc['annual_growth'] / 100))
            # TAM_used_cumulative_fc is NOT reset; it's a global sum of acquired users against an evolving TAM ceiling.
        ...
        else: # Subsequent months
            # Base for new acquisitions is the cumulative *newly acquired* base from previous months
            potential_new_acquisitions_this_month_fc = int(cumulative_acquired_base_fc * acquisition_rate_fc)

        # Apply TAM Cap if enabled
        actual_new_acquisitions_this_month_fc = potential_new_acquisitions_this_month_fc
        if enforce_cap_growth_fc:
            # Check if adding potential new users exceeds the current year's TAM limit
            if (TAM_used_cumulative_fc + potential_new_acquisitions_this_month_fc) > TAM_current_year_fc:
                actual_new_acquisitions_this_month_fc = max(0, TAM_current_year_fc - TAM_used_cumulative_fc)
        
        TAM_used_cumulative_fc += actual_new_acquisitions_this_month_fc
        cumulative_acquired_base_fc += actual_new_acquisitions_this_month_fc # Update base with *actually acquired* new users
        
        newly_acquired_this_month_fc_val = actual_new_acquisitions_this_month_fc # This is the count of new users for this month
        # --- END: Monthly New User Acquisition Calculation ---

        durations_for_this_year_fc = yearly_duration_share.get(current_year_num_fc, {})
        rejoining_users_this_month_fc_val = rejoin_tracker_fc.get(m_idx_fc, 0) # Total rejoining users available this month
        
        total_onboarding_this_month_fc = newly_acquired_this_month_fc_val + rejoining_users_this_month_fc_val

        # Temporary counter for allocating rejoining users to cohorts this month
        ...
        for dur_val_fc, dur_share_pct_fc in durations_for_this_year_fc.items():
            ...
            # Users for this duration, from total onboarding pool
            ...
            for installment_val_fc, slab_share_pct_fc in slab_map[dur_val_fc].items():
                # Users for this slab, from duration's user pool
                ...
                for slot_num_fc, slot_config_meta_fc in slot_fees[dur_val_fc].items(): # slot_num_fc is 1-indexed
                    ...
                    # Users for this specific cohort (Duration/Slab/Slot)
                    users_in_this_specific_cohort_fc = int(users_for_this_slab_fc * (slot_distribution[dur_val_fc].get(slot_num_fc, 0) / 100))

                    if users_in_this_specific_cohort_fc == 0: continue
                    
                    # Allocate users for this cohort from rejoining pool first, then from new acquisitions pool
                    # This is for logging/tracking the source of users in the cohort.
                    ...
                    temp_rejoining_users_for_allocation -= from_rejoin_pool_fc # Deplete available rejoins for this month
                    ...
                    lifecycle_data_fc.append({"Month": current_month_num_fc, 
                                            ...
                                            "Total Onboarding to Cohort": users_in_this_specific_cohort_fc}) # users_in_this_specific_cohort_fc is the sum of from_newly_acquired_fc and from_rejoin_pool_fc for this cohort

                    rejoin_at_month_idx_fc = m_idx_fc + dur_val_fc + int(current_rest_period_months_fc)
                    # Users who complete the cycle (non-defaulters) are eligible to rejoin
        ...
        # The old logic for populating `new_users_monthly_fc.append(...)` is removed as new user calculation is now at the start of the loop.
        ...
        if not df_forecast_main.empty:
            ...
            df_monthly_direct_main = df_forecast_main.groupby("Month Joined")[
                ["Cash In (Installments This Month)", "NII Earned This Month (Avg)", "Pools Formed", "Users"] 
                                                  ...
                                                  "Users": "Users Joining This Month", # This 'Users' is sum of all cohort users joined in this month
